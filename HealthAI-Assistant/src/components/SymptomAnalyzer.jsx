...
const getRetryDelayFromErrorMessage = (message = '') => {
...
export default function SymptomAnalyzer() {
    ...
    useEffect(() => {
        ...
        if (lastWord.length >= 2) {
            const matches = SYMPTOM_DATABASE.filter(symptom => symptom.toLowerCase().includes(lastWord.toLowerCase())).slice(0, 8); // Limit to 8 suggestions
    ...
    const handleAnalyze = async (e) => {
        ...
        if (retryAfterSeconds > 0) {
            const cooldownMessage = {
                ...
                role: 'assistant',
                content: `Gemini API is temporarily rate-limited for this project. Please wait about ${retryAfterSeconds} second${retryAfterSeconds === 1 ? '' : 's'} before trying again.`,
                timestamp: new Date(),
            };
        ...
        try {
            ...
            const retryDelay = getRetryDelayFromErrorMessage(friendlyMessage);
            if (retryDelay > 0) {
            ...
            errorMessage.content = friendlyMessage;
    ...
    const handleKeyDown = (e) => {
        if (showAutocomplete && filteredSymptoms.length > 0) {
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                setSelectedIndex(prev => prev < filteredSymptoms.length - 1 ? prev + 1 : prev);
            }
    ...
    return (<div className="flex flex-col h-screen bg-background">
      ...
      {/* Header */}
      <div className="relative bg-background/80 backdrop-blur-xl border-b border-border/60 shadow-lg">
        ...
        <div className="relative px-3 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-center">
            ...
            {messages.length > 0 && !loading && (<button onClick={handleNewSession} className="absolute right-3 sm:right-6 top-1/2 -translate-y-1/2 flex items-center gap-1.5 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-xs sm:text-sm font-medium shadow-md hover:shadow-lg hover:scale-105 transition-all duration-200" title="Start New Session">
              ...
              <div>
                ...
                <p className="text-[10px] sm:text-xs md:text-sm text-gray-600 dark:text-gray-400">
                  {loading ? (<span className="flex items-center justify-center gap-1">
                    ...
                    </span>) : ('AI-powered health analysis')}
                </p>
              </div>
            </div>
          </div>
      ...
      {showEmergencyBanner && (<div className="bg-red-500 text-white px-3 sm:px-4 md:px-6 py-2 sm:py-3 flex items-start sm:items-center gap-2 sm:gap-3 shadow-lg animate-slide-down">
      ...
      <div className={`flex-1 ${messages.length > 0 && "overflow-y-auto scrollbar-custom"} px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 space-y-3 sm:space-y-4 md:space-y-6 scroll-smooth`}>
        {messages.length === 0 && (<div className="h-full flex items-center justify-center px-2 sm:px-4">
            <div className="text-center space-y-3 sm:space-y-4 md:space-y-6 max-w-xs sm:max-w-md md:max-w-2xl animate-fade-in">
              ...
              {recentSearches.length > 0 && (<div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 justify-center">
                    {recentSearches.map((search, idx) => (<button key={idx} onClick={() => setInput(search)} className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-[10px] sm:text-xs text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all duration-200 border border-blue-200 dark:border-blue-800 max-w-[150px] sm:max-w-none truncate">
        ...
        {messages.map((message) => (<div key={message.id} className={`flex gap-2 sm:gap-3 animate-slide-up ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`group relative max-w-[90%] sm:max-w-[85%] md:max-w-[75%] lg:max-w-[70%] ${message.role === 'user'
                ? 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-xl shadow-blue-500/30'
                : 'bg-card/70 text-foreground border border-border/60 shadow-xl shadow-black/40'} rounded-xl sm:rounded-2xl px-3 py-2 sm:px-4 sm:py-3 md:px-5 md:py-3 transition-all duration-300 hover:scale-[1.02]`}>
              {message.role === 'assistant' ? (<div className="prose prose-xs sm:prose-sm md:prose-base dark:prose-invert max-w-none">
                  <ReactMarkdown>{message.content}</ReactMarkdown>
                </div>) : (<p className="text-xs sm:text-sm md:text-[15px] leading-relaxed whitespace-pre-wrap break-words">
                  {message.content}
                </p>)}
            </div>
      ...
      <div className="sticky bottom-16 sm:bottom-20 border-t border-border/60 bg-background/90 backdrop-blur-xl p-2 sm:p-3 md:p-4 lg:p-5 shadow-2xl">
        <form onSubmit={handleAnalyze} className="max-w-6xl mx-auto space-y-2 sm:space-y-3">
          
          {/* Quick Symptom Chips - Responsive */}
          <div className="flex flex-wrap gap-1.5 sm:gap-2 justify-center">
            {COMMON_SYMPTOMS.map((symptom) => (<button key={symptom} type="button" onClick={() => handleQuickSymptom(symptom)} className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-full bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 text-[10px] sm:text-xs font-medium text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 hover:from-blue-100 hover:to-indigo-100 dark:hover:from-blue-900/30 dark:hover:to-indigo-900/30 hover:scale-105 hover:shadow-md transition-all duration-200">
                {symptom}
              </button>))}
          </div>

          {/* Main Input Container - Responsive */}
          <div className="relative flex items-end gap-1.5 sm:gap-2 bg-card/70 rounded-xl sm:rounded-2xl shadow-2xl shadow-black/40 p-1.5 sm:p-2 border-2 border-border/60 focus-within:border-primary focus-within:shadow-primary/20 transition-all duration-300">
            
            {/* Autocomplete Dropdown */}
            {showAutocomplete && filteredSymptoms.length > 0 && (<div ref={autocompleteRef} className="absolute bottom-full left-0 right-0 mb-2 bg-popover rounded-xl shadow-2xl border-2 border-border/60 max-h-64 overflow-y-auto z-50 animate-fade-in">
                <div className="p-2 border-b border-border/60 bg-primary/10">
                  <p className="text-xs font-semibold text-muted-foreground flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-blue-500"/>
                    Suggested Symptoms
                  </p>
                </div>
                {filteredSymptoms.map((symptom, index) => (<button key={index} type="button" onClick={() => handleSelectSymptom(symptom)} className={`w-full text-left px-4 py-2.5 text-sm transition-colors duration-150 ${index === selectedIndex
                    ? 'bg-blue-500 text-white'
                    : 'text-foreground hover:bg-muted'} ${index === filteredSymptoms.length - 1 ? '' : 'border-b border-border/60'}`}>
                    <span className="font-medium">{symptom}</span>
                  </button>))}
              </div>)}
            
            {/* Voice Input Button with Pulse Animation - Responsive */}
            <button type="button" onClick={startVoiceInput} disabled={loading} className={`flex-shrink-0 p-2 sm:p-2.5 md:p-3 rounded-lg sm:rounded-xl transition-all duration-300 ${isListening
            ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/50 scale-110'
            : 'bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 text-gray-600 dark:text-gray-300 hover:from-blue-50 hover:to-indigo-50 dark:hover:from-blue-900/30 dark:hover:to-indigo-900/30 hover:text-blue-600 dark:hover:text-blue-400 hover:scale-105'} disabled:opacity-50 disabled:cursor-not-allowed`} title={isListening ? 'Stop listening' : 'Start voice input'}>
              {isListening ? (<div className="relative">
                  <MicOff className="w-4 h-4 sm:w-5 sm:h-5"/>
                  <span className="absolute inset-0 rounded-full bg-blue-400 animate-ping opacity-75"></span>
                </div>) : (<Mic className="w-4 h-4 sm:w-5 sm:h-5"/>)}
            </button>

            {/* Textarea - Responsive */}
            <textarea ref={textareaRef} value={input + interimText} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Describe your symptoms..." rows={1} disabled={loading} className="flex-1 resize-none bg-transparent border-none outline-none px-2 py-2 sm:px-3 sm:py-2.5 md:py-3 text-sm sm:text-base text-gray-800 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 max-h-24 sm:max-h-32 disabled:opacity-50 font-medium" style={{ minHeight: '40px' }}/>

            {/* Clear Button - Responsive */}
            {input && !loading && (<button type="button" onClick={handleClearInput} className="flex-shrink-0 p-1.5 sm:p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-all duration-200" title="Clear input">
                <X className="w-4 h-4 sm:w-5 sm:h-5"/>
              </button>)}
            
            {/* Submit Button - Prominent & Responsive */}
            <button type="submit" disabled={!input.trim() || loading || retryAfterSeconds > 0} className="flex-shrink-0 px-3 py-2 sm:px-4 sm:py-2.5 md:px-5 md:py-3 rounded-lg sm:rounded-xl bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-600 text-white text-sm sm:text-base font-semibold shadow-lg shadow-blue-500/40 hover:shadow-xl hover:shadow-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 hover:scale-105 active:scale-95 disabled:hover:scale-100">
              {loading ? (<div className="flex items-center gap-1.5 sm:gap-2">
                  <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin"/>
                  <span className="hidden sm:inline">Analyzing</span>
                </div>) : (<div className="flex items-center gap-1.5 sm:gap-2">
                  <Send className="w-4 h-4 sm:w-5 sm:h-5"/>
                  <span className="hidden sm:inline">{retryAfterSeconds > 0 ? `Retry in ${retryAfterSeconds}s` : 'Analyze'}</span>
                </div>)}
            </button>
          </div>
          ...
          <div className="flex items-center justify-center gap-1.5 sm:gap-2 text-center px-2">
            <div className="relative group">
              <button type="button" onMouseEnter={() => setShowInfoTooltip(true)} onMouseLeave={() => setShowInfoTooltip(false)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
                <Info className="w-3 h-3 sm:w-4 sm:h-4"/>
              </button>
              {showInfoTooltip && (<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 sm:w-64 p-2 sm:p-3 bg-popover text-popover-foreground text-[10px] sm:text-xs rounded-lg shadow-xl z-50 animate-fade-in border border-border/60">
